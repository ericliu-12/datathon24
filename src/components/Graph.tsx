...
export default function Graph() {
  ...
  const handleNodeMouseUp = (index: number) => {
  ...
  const handleNodeClick = (index: number) => {
    if (connectingFrom === null) {
      ...
      setConnections([...connections, [connectingFrom, index]]);
  ...
  return (
    <div className="relative min-h-screen">
      ...
      {connections.map(([from, to], index) =>
        to !== null &&
        from !== null &&
        nodeRefs[from]?.current &&
        nodeRefs[to]?.current ? (
          <Connector key={index} refA={nodeRefs[from]} refB={nodeRefs[to]} />
        ) : null
      )}
    </div>
  );
